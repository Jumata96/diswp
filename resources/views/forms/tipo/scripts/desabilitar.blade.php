<script type="text/javascript">
  //----JPaiva-07-08-2018------------------DESABILITAR---------------------------
    @foreach ($tipo as $val)
        $('#d{{$val->idtipo}}').click(function(e){
          e.preventDefault();

          id = $(this).data('iddesabilitar');

          $.ajax({
                url: "{{ url('/tipo/desabilitar') }}",
                type:"POST",
              ...
              if ( data[0] == "error") {
                ...
                "<td class='center'>"+
                  ...
                  " <a href='#confirmacion3"+ obj[0]['idtipo'] +"' class='btn-floating waves-effect waves-light grey lighten-5 tooltipped modal-trigger' data-position='top' data-delay='500' data-tooltip='Habilitar'><i class='material-icons' style='color: #2e7d32'>check</i></a>"+
                "</td>"
                );
              }
